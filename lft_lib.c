...
const int minpacklen = 60;     /* minimum user-supplied probe length */
...
const int hop_info_size = 256;  /* can't be more than this */

const unsigned int max_net_dev_input = 64; /* only take this much input */

const int def_payload_len = 10;     /* default payload length for UDP packets */

#if defined( __CYGWIN__ ) || defined( WIN32 ) || defined(_WIN32)
static int sock = -1;
#endif

int global_output_style=0;	/* 0 - ordinary output, 1 - xml output */
...
static void LFTDefaultErrorHandler(lft_session_params * sess, int code, const void * params)
{
	...
	case WRN_OCHECK_OPEN_SOCK:
		if(!global_output_style && sess->noisy>1)
    ...
    if(ehip->tp->recv.tv_sec) {
        if (ehip->last_hop.s_addr != ehip->tp->hopaddr.s_addr) {
            ...
            */
			//if(!global_output_style)
			//	printf(" ");
            print_host (sess, ehip->tp->hopaddr);//ehip->tp->hopaddr
...
if(global_output_style == 1)
		{
			if(ind==';')
				printf (";%.2f", timediff_ms(ehip->tp->sent, ehip->tp->recv));
			else
				printf (" timems=\"%.2f", timediff_ms(ehip->tp->sent, ehip->tp->recv));
		}
else if(global_output_style == 2)
{
	if(ind=='/')		
		...
		printf("\"lfttime\":\"%.2f",timediff_ms(ehip->tp->sent, ehip->tp->recv));
}
else
		printf ("%c%.2f", ind, timediff_ms(ehip->tp->sent, ehip->tp->recv));
...
{
	  char pingcmd[100];
	  char *pingip=(char*)inet_ntoa(ipaddress);
    ...
    case EVT_TRACE_REPORT_START:
		if(!global_output_style)
			printf ("[");//printf ("TTL LFT trace to ");
	// if(global_output_style == 2)
		...
		else if(global_output_style == 2)
		{
			...
			{
				case 0:
					printf("\"protocol\":\"tcp\",\"port num\":%d,",sess->dport);
					break;
				case 4:
					printf("\"protocol\":\"tcp\",\"port num\":\"%d-%d\",",(sess->dport + (*((const int *)params))),sess->dport);
					break;
				case 1:
					printf("\"protocol\":\"udp\",\"port num\":\"%d-%d\",",(sess->dport + (*((const int *)params))),sess->dport);
					break;
				case 2:
					printf("\"protocol\":\"icmp\",");
					break;
				case 3:
					printf("\"protocol\":\"rfc1393\",");
					break;
			}
			printf ("\"trace\":[\n");
			fflush(stdout);
		...
		else
		{
			...
			{
				case 0:
					printf("%d tcp",sess->dport);
					break;
				case 4:
					printf("%d-%d tcp",sess->dport, (sess->dport + (*((const int *)params))));
					break;
				case 1:
					printf("%d-%d udp",sess->dport, (sess->dport + (*((const int *)params))));
					break;
				case 2:
					printf("0 icmp");
					break;
    ...
    case EVT_RPT_NO_REPLY:
		...
		{
			if (nrparam->noreply == 1 && nrparam->hopno == 1)
				printf("{\"seq\":%d,\"type\":\"neglected\",\"note\":\"no reply packets received from TTL\"}\n",nrparam->hopno);
			if (nrparam->noreply > 1)
				for(nc=(nrparam->hopno - nrparam->noreply) + 1;nc<(nrparam->hopno)+1;nc++)
					printf(",{\"seq\":%d,\"type\":\"neglected\",\"note\":\"no reply packets received from TTL\"}\n",nc);
				type_sign = 1;
		}
		else			
    ...
    case EVT_RPT_FRW_INSPECT_PACKS:
		if(global_output_style == 1)
			...
			printf(",{\"type\":\"firewall\",\"note\":\"the next gateway may statefully inspect packets\"}\n");
		else
    ...
    case EVT_RPT_FRW_STATE_FILTER:
		if(global_output_style == 1)
			...
			printf(",{\"type\":\"firewall\",\"note\":\"the next gateway may implement a flag-based state filter\"}\n");
		else
    ...
    case EVT_RPT_BSD_BUG:
		if(global_output_style == 1)
			...
			printf(",{\"type\":\"4.2-3 BSD bug\",\"note\":\"the next gateway may errantly reply with reused TTLs\"}\n");
		else
    ...
    case EVT_RPT_PACKET_LIST_END:
		if(global_output_style == 1)
		...
		else
			{
				...
				{
					printf(" ms\",");
					sprintf(pingcmd,"echo -ne \"`ping %s -w 1 -c 1|sed -n '2p'|cut -d' ' -f7|cut -d'=' -f2`\"",pingip);
					printf ("\"pingtime\":\"");
					if(system(pingcmd)==-1)
					//printf("0 ");
					perror("ping failed!\n");
    ...
    case EVT_RPT_HOP_INFO_START:
		if(global_output_style == 1)
			...
			{
				if((*((const int *)params)) != 0)
					printf(",{\"seq\":%d,",(*((const int *)params)) + 1);
				else
					printf("{\"seq\":%d,",(*((const int *)params)) + 1);
				type_sign = 1;
			}
    ...
    case EVT_RPT_NO_HOPS:
		...
		else if(global_output_style == 2)
		{
			if (type_sign==1)
				printf(",");
			if (sess->target_anomaly)
				printf("{\"type\":\"%d/tcp sequence anomaly from target\",\"note\":\"Try advanced options to see packets\"}\n",sess->dport);
			else if (sess->protocol==1)
				printf("{\"type\":\"%d-%d/udp no reply from target\",\"note\":\"Try advanced options to see packets\"}\n",sess->dport, (sess->dport + (*((const int *)params))));
			else if (sess->protocol==0)
				printf("{\"type\":\"%d/tcp no reply from target\",\"note\":\"Try advanced options to see packets\"}\n",sess->dport);
			else if (sess->protocol==4)
				printf("{\"type\":\"%d-%d/tcp no reply from target\",\"note\":\"Try advanced options to see packets\"}\n",sess->dport, (sess->dport + (*((const int *)params))));
			else
				printf("{\"type\":\"icmp no reply from target\",\"note\":\"Try advanced options to see packets\"}\n");
		}
		else
		{
			if (sess->target_anomaly)
    ...
    case EVT_RPT_TIME_TRACE:
		...
		else if(global_output_style == 2)
		{
			if(sess->timetrace)
				printf("]");
			printf (",\"finishtime\":\"%s ms\"", tbuf);
			printf (",\"elapsed\":\"%.2f ms\"",(timediff_ms(sess->begin_time, sess->now) / 1000));
			printf (",\"tracingtime\":\"%.2f ms\"", 
					(timediff_ms(sess->begin_time, sess->trace_done_time) / 1000));
			if (sess->resolve_names || sess->do_aslookup || sess->do_netlookup) 
				printf(",\"resolvingtime\":\"%.2f ms\"", (timediff_ms(sess->trace_done_time, sess->now) / 1000));
			//printf(" />\n");
		}
		else
		{
...
#endif
					(void)strftime(tbuf, sizeof(tbuf), time_format, (struct tm *) localtime((time_t *) &(sess->begin_time.tv_sec)));
				if(global_output_style == 0)
					printf ("LFT trace started at %s\n", tbuf);
				else if(global_output_style == 2)
					printf("{\"tool\":\"lft\",\"begin time\":\"%s\",", tbuf);
			}
			if (!sess->nostatus && !sess->noisy)	
				if(global_output_style == 0)
					printf("Tracing ");
				else if(global_output_style == 2 && (!sess->timetrace) )
					printf("{\"tool\":\"lft\",");
